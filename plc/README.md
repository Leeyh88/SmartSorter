...
- **PLC 입력 COM**: 외부 $24{V}$ SMPS의 **`+24V`** 연결.
- **포토센서 / 스위치**: 동작 시 **`0V (24G)`** 신호를 PLC 입력 단자로 인가하여 도통(Loop 형성) 감지.

### 2️⃣ 출력부 배선 (릴레이 출력 ➜ 포토커플러 중계)
- **PLC 출력 COM**: 외부 $24{V}$ SMPS의 **`+24V`** 연결. (스위칭 시 단자에서 $+24{V}$ 출력)
- **포토커플러 모듈 입력측**:
  - `IN1 ~ IN4`: PLC 출력 단자(`P0021 ~ P0024`)에서 들어오는 **`+24V`** 연결.
  - `포토커플러 입력 COM`: 외부 $24{V}$ SMPS의 **`0V (24G)`** 연결.
- **포토커플러 모듈 출력측 (아두이노 $5{V}$ 회로 절연)**:
  - `VCC` / `GND`: 아두이노의 $5{V}$ / **`GND`** 연결.
