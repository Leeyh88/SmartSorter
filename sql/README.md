...
1. **SQL Server Management Studio (SSMS)** 실행 후 서버에 접속합니다.
2. 새 쿼리 창을 열고 `sql/schema.sql` 파일의 내용을 전체 복사하여 붙여넣습니다.
3. **`실행 (F5)`**을 누르면 데이터베이스(`SmartSorterDB`), 테이블 3종, 인덱스 및 초기 설정 데이터가 한 번에 안전하게 생성됩니다.
